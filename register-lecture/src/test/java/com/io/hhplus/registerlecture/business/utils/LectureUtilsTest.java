...
class LectureUtilsTest {
    ...
    void test_getOneByPrimaryId_when_data_is_empty() {
        // given
        long id = 9;
        Lecture expectedLecture = lectureUtils.empty();
        given(lectureRepository.findById(anyLong())).willReturn(Optional.of(expectedLecture));
        // when
        Lecture realLecture = lectureUtils.getOneByPrimaryId(id);
        // then
        assertEquals(-1, realLecture.getId());
    }
    ...
    void meets_validateBeforePersist_when_lecture_is_null_then_return_EMPTY() {
        ...
        // when
        ProcessResultDto realProcessResult = lectureUtils.validateBeforePersist(lecture);
        // then
    ...
    void meets_validateBeforePersist_when_id_is_negative() {
        ...
        // then
        ProcessResultDto realProcessResult = lectureUtils.validateAfterPersist(lecture);
    }
    ...
    void meets_validBeforePersist_when_lectureName_is_null_or_blank() {
        ...
        // when
        ProcessResultDto realProcessResult1 = lectureUtils.validateBeforePersist(lecture1);
        ProcessResultDto realProcessResult2 = lectureUtils.validateBeforePersist(lecture2);
        // then
    ...
    void test_write() {
        ...
        Lecture lecture = Lecture.builder()
                ...
                .useYn("Y")
                .build();
        doReturn(lecture).when(lectureRepository).save(any(Lecture.class));
        // when
        Lecture savedLecture = lectureUtils.write(lecture);
        // then
    ...
    void meets_validateAvailableDataByPrimaryId_when_lecture_is_not_usable_then_return_ILLEGAL_STATE() {
        ...
        Lecture lecture = Lecture.builder()
                ...
                .build();
        doReturn(lecture).when(lectureRepository).save(any(Lecture.class));
        given(lectureRepository.findById(id)).willReturn(Optional.of(lecture));
        // when
        Lecture savedLecture = lectureUtils.write(lecture);
        ProcessResultDto processResultDto = lectureUtils.validateAvailableDataByPrimaryId(savedLecture.getId());
        // then
    ...
    void meets_validateAvailableDataByPrimaryId_when_lecture_is_not_found_then_return_ILLEGAL_STATE() {
        // given
        Optional<Lecture> optionalLecture = Optional.of(lectureUtils.empty());
        long id = 9999;
    ...
    void meets_validateAvailableDataByPrimaryIdAndOptionId_when_lecture_already_began() {
        ...
        LectureOption lectureOption = LectureOption.builder()
                ...
                .build();
        doReturn(lectureOption).when(lectureOptionRepository).save(any(LectureOption.class));
        given(lectureOptionRepository.findByIdAndLectureId(anyLong(), anyLong())).willReturn(Optional.of(lectureOption));
        // when
        LectureOption savedLectureOption = lectureUtils.write(lectureOption);
        ProcessResultDto processResultDto = lectureUtils.validateAvailableDataByPrimaryIdAndOptionId(savedLectureOption.getLectureId(), savedLectureOption.getId());
        // then
        assertEquals(ProcessCode.ILLEGAL_STATE, processResultDto.getProcessCode());
